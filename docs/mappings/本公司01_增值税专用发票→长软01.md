...
function processInvoice01(xmlBody) {
  ...
  if (tspzbz === '04') {
    ...
    return {
      ...
      // 其他字段正常映射
      amount_excluding_tax: parseFloat(xmlBody.JE),
      tax_amount: parseFloat(xmlBody.SE),
      amount_including_tax: parseFloat(xmlBody.JSHJ),
...
function validateInvoice01Amount(body) {
  ...
  return {
    ...
    amount_including_tax: totalAmount,
    is_valid: diff <= 0.02
  };
}
```
...
2. 长软API文档: `【长软】发票查验服务调用说明V4.1.2.pdf` (第4-5页, 第29页)
