# 本公司87_数电纸质发票(机动车销售统一发票)→长软09(QDLX=03) 字段映射表

**文档版本**: v1.0
**创建日期**: 2025-12-30
**发票类型**: 87 - 数电纸质发票（机动车销售统一发票）

---

## 📋 概述

本文档定义了本公司发票查验API与长软发票查验服务之间,针对"数电纸质发票（机动车销售统一发票）"的完整字段映射关系。
...
| 项目 | 本公司API | 长软API | 说明 |
|------|-----------|---------|------|
| 发票类型代码 | `87` | `09` | 长软使用统一的09类型表示数电发票 |
| 发票类型名称 | 纸质发票（机动车销售统一发票） | 数电发票(电子机动车销售统一发票) | 数电纸质版本 |
| 数据结构定义 | `#/definitions/231697060` | `3.2.3.15` | 与类型83共用相同schema |
| 数电发票清单类型 | - | `QDLX=03` | **关键区分**:03表示机动车 |
| 发票号码特征 | 纸质发票号码(8位) | 8位数字 | 纸质发票号码 |
| 发票代码特征 | 12位纸质发票代码 | 12位 | 数电纸质发票有发票代码 |
...
|---|---|---|---|---|---|---|
| `invoice_number` | 发票号码 | string(20) | `BODY/FPHM` | 发票号码 | VARCHAR2(20) | **纸质发票号码**,8位数字 |
| `invoice_code` | 纸质发票代码 | string(12) | `BODY/FPDM` | 发票代码 | VARCHAR2(12) | **12位发票代码**,数电纸质发票特有 |
| `paper_invoice_no` | 纸质发票号码 | string(20) | `BODY/FPHM` | 发票号码 | VARCHAR2(20) | 同invoice_number,8位纸质发票号码 |
...
| `vat_tax_amount` | 税额 | number(18,6) | `BODY/ZZSSE` | 增值税税额 | VARCHAR2(32) | 类型转换: 长软为字符串 |
| `amount_including_tax` | 价税合计 | number(18,2) | `BODY/JSHJ` | 价税合计 | VARCHAR2(32) | 类型转换: 长软为字符串 |
| `tax_amount` | 合计税额 | number(18,2) | `BODY/ZZSSE` | 增值税税额 | VARCHAR2(32) | 映射自ZZSSE,机动车只有一个税额 |
| `amount_in_words` | 价税合计(大写) | string(300) | - | - | - | **本公司独有**,需从业务系统计算 |
...
| `issuer` | 开票人 | string(300) | - | - | - | **本公司独有**,纸质发票可能有开票人 |
...
| 本公司字段 | 数据来源 | 处理逻辑 |
|-----------|---------|----------|
| `amount_in_words` | 计算得出 | 将amount_including_tax转换为大写中文金额 |
...
转换为本公司API:
{
  ...
  "amount_including_tax": 113000.00,
  "tax_amount": 13000.00,
...
| 字段 | 本公司长度 | 长软长度 | 处理策略 |
|------|-----------|---------|----------|
| invoice_number | 20 | 20 | ✅ 本公司更大,纸质发票号码实际8位 |
| invoice_code | 12 | 12 | ✅ 长度一致,纸质发票代码12位 |
...
**处理规则**: 数电纸质发票的代开信息可能与传统发票不同,建议记录但不作为主要业务字段。
...
- 如果有 `original_blue_invoice_no` 或 `original_blue_paper_invoice_code` 字段,则为红字发票
- 否则为蓝字发票
...
1. **数电纸质发票特征**
   - 纸质发票代码为12位,纸质发票号码为8位
   - 通过长软09类型查验,QDLX=03区分为机动车
   - 有实体纸质票据,但归属于数电发票体系

2. **与数电电子机动车发票(83)的区别**
   - 类型83: 无发票代码,发票号码20位数字,纯电子
   - 类型87: 有发票代码(12位),发票号码8位,有纸质实体
   - 都通过长软09(QDLX=03)查验
...
7. **与传统机动车发票(03)的区别**
   - 类型87: 数电纸质,通过长软09(QDLX=03)查验
   - 类型03: 传统纸质,通过长软03查验
   - 两者都有12位发票代码和8位发票号码

### 6.2 与其他类型的区别

| 特性 | 纸质机动车数电(87) | 电子机动车数电(83) | 传统机动车(03) |
|------|------------------|------------------|--------------|
| 发票号码 | 8位数字 | 20位数字 | 8位数字 |
| 发票代码 | 12位 | 无 | 12位 |
| 长软类型 | 09(QDLX=03) | 09(QDLX=03) | 03 |
| 明细结构 | ❌ 无明细 | ❌ 无明细 | ❌ 无明细 |
| 特殊字段 | VIN码、发动机号、合格证号 | VIN码、发动机号、合格证号 | VIN码、发动机号、合格证号 |
| 购方标识 | 税号或身份证号 | 税号或身份证号 | 税号或身份证号 |
| 介质 | 纸质(数电体系) | 纯电子 | 传统纸质 |
| 发票体系 | 数电发票 | 数电发票 | 传统发票 |
...
```javascript
// 处理示例
if (GFSBH && GFSBH.length >= 15) {
  buyer_tax_no = GFSBH;  // 企业税号
} else if (SFZHM && SFZHM.length === 18) {
...
- 金额为负值
- 有 `original_blue_paper_invoice_code` 或 `original_blue_paper_invoice_no` 字段
...
**纸质发票特有字段**:
- `invoice_code`: 12位纸质发票代码,必填
- `paper_invoice_no`: 8位纸质发票号码,与invoice_number相同
- `invoice_number`: 也存储8位纸质发票号码

**字段映射策略**:
```javascript
{
  invoice_code: FPDM,           // 12位发票代码
  invoice_number: FPHM,         // 8位发票号码
  paper_invoice_no: FPHM        // 同invoice_number
}
```
...
|------|-----------|---------|------------|
| invoice_code | 12 | 12 | ✅ 长度一致,安全 |
| invoice_number | 20 | 20 | ✅ 本公司更大,纸质发票号码实际8位,安全 |
...
**注意**:
- 类型87使用纸质发票代码+号码查验
- 金额使用不含税价(price_without_tax),而非价税合计
- 与类型83(20位数电发票号码+价税合计)不同
...
- QDLX=20: 数电专票(本公司81)
- QDLX=10: 数电普票(本公司82)
- QDLX=03: 数电机动车(本公司83、87) ← **本类型**
- QDLX=15: 数电二手车(本公司84、88)
...
- 类型83和87都映射到QDLX=03,通过发票代码是否存在区分
- 记录QDLX值用于日志和调试

#### 7.4.3 类型83 vs 类型87区分

**如何区分类型83和87**:
```javascript
// 长软返回的QDLX都是03,需通过发票代码区分
if (QDLX === '03') {
...
- `vat_tax_amount` ← ZZSSE (字符串转数值)
- `tax_amount` ← ZZSSE (同vat_tax_amount,机动车只有一个税额)
- `amount_including_tax` ← JSHJ (字符串转数值)
...
```javascript
{
  ...
  vat_tax_amount: parseFloat(ZZSSE),              // 13000.00
  tax_amount: parseFloat(ZZSSE),                  // 13000.00
  amount_including_tax: parseFloat(JSHJ)          // 113000.00
}
```
...
**发票代码和号码处理**:
- `invoice_code` = FPDM (12位纸质发票代码,必填)
- `invoice_number` = FPHM (8位纸质发票号码,必填)
- `paper_invoice_no` = FPHM (同invoice_number)
...
// 税务总局查验平台
{
  invoice_code: invoice_code,      // 12位
  invoice_number: invoice_number,  // 8位
  issue_date: issue_date,
  amount: price_without_tax        // 注意:使用不含税价
}
...
1. 发票不存在: `CYJGDM ≠ 001`
2. 发票已作废: `ZFBZ = 1`
3. 金额不匹配: 查验时传入的金额与实际不符(注意使用不含税价)
4. 类型不匹配: QDLX ≠ 03
5. 发票代码号码错误: 发票代码或号码格式不正确

**建议处理**:
- 记录完整的长软返回XML用于排查
- 记录QDLX值,确保类型匹配
- 对于作废发票,仍返回完整信息但标注状态
- 金额验证失败时,提示用户检查是否使用不含税价
- 发票代码号码验证失败时,检查格式(代码12位,号码8位)

---

## 参考文档

1. 本公司API定义: [`/partners/invoice-verifications`](https://s.apifox.cn/2795ad32-31e7-4363-8294-578e8dc6202c/395314528e0) - schema `#/definitions/231697060`
2. 长软API文档: `【长软】发票查验服务调用说明V4.1.2.pdf` - 第3.2.3.15节
3. 通用枚举映射: `00_通用说明_枚举映射.md`
4. 通用请求参数映射: `00_通用说明_请求参数映射.md`
5. 数电电子机动车发票映射(对比参考): `本公司83_机动车销售电子统一发票→长软09(QDLX=03).md`
6. 传统机动车发票映射(对比参考): `本公司03_机动车销售统一发票→长软03.md`
