...
**版本更新说明**:
- v1.1 (2025-12-31): 新增BXF（保险费）字段映射，对应长软API V4.1.2更新
...
| `tax_amount` | 合计税额 | number(18,2) | `BODY/SE` | 税额 | VARCHAR2(32) | 类型转换：number ↔ string |
| `amount_including_tax` | 价税合计 | number(18,2) | `BODY/JSHJ` | 价税合计 | VARCHAR2(32) | 类型转换：number ↔ string |
| `amount_excluding_tax` | 合计金额(不含税) | number(18,2) | - | - | - | **本公司独有**，可从JSHJ-SE反推 |
| `amount_in_words` | 价税合计（大写） | string(100) | - | - | - | **本公司独有**，数字大写转换 |
...
| `flight_specific_elements.insurance_fee` | 保险费 | string(20) | `BODY/BXF` | 保险费 | VARCHAR2(32) | **🆕 V4.1.2新增** 类型转换：string ↔ number |
...
```
价税合计（JSHJ） = 票价（PJ） + 燃油附加费（RYFJF） + 民航发展基金（MHFZJJ） + 其他税费（QTSF） + 保险费（BXF）
                  （注：BXF字段为V4.1.2新增，旧版本中此项为0或不存在）
...
- 部分特定要素字段（endorsement, verification_code, tips等） - 长软不返回，需从业务系统获取
- 航段的ticket_effective_date、valid_until_date、free_baggage_allowance - 长软不返回

**V4.1.2版本新增长软返回字段**:
- `BXF` (保险费) - 🆕 新增，之前版本长软不返回此字段
...
2. **费用计算验证**:
   - 必须验证：PJ + RYFJF + MHFZJJ + QTSF + BXF = JSHJ
   - 注意：BXF（保险费）为V4.1.2新增字段，旧版本中可能不存在或为空，验证时需兼容处理
   - 如果不相等，需记录警告日志
...
4. **缺失字段处理**:
   ...
   - 🆕 **V4.1.2版本兼容性**: BXF（保险费）字段在旧版本中不存在，需做空值兼容处理
